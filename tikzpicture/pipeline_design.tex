...
% Row 1: Inputs
\node[io] (apk) {\textbf{Target APK}\\\texttt{base.apk} +\\extract native libs};
\node[io, right=of apk] (poirot) {\textbf{POIROT}\\Fuzzing};

% Row 2: Parsing / setup
\node[proc, below=of poirot] (crashparser) {\textbf{Crash parser}\\\texttt{CrashSummary}};
\node[proc, below=of apk] (setup) {\textbf{Initialization}\\Jadx + Ghidra};
...
\path[line] (setup) -- node[left,pos=0.45]{Load APK and libs} (agent);
...
% Legend
\node[block, minimum width=1.8cm, minimum height=0.8cm, above=4.7cm of report, xshift=-0.3cm] (legend1) {};
