...
The full source code is available at \href{https://github.com/Nicola-01/LLM-Triaging}{\texttt{github.com/Nicola-01/LLM-Triaging}}.
...
\begin{myitemize}
    ...
    Provides the agent abstraction, the \gls{mcp}
...
\begin{itemize}
    ...
    The corresponding system prompt is provided in Listing~\ref{lst:appmetadata-prompt}.
    ...
    Listing~\ref{lst:systemPrompt} shows the full prompt.
    ...
    Further details are provided in Section~\ref{chp:mcp-shimming}.
    
\end{itemize}

\begin{lstlisting}[language={},
caption={System prompt used for extracting application metadata via Jadx MCP.},
label={lst:appmetadata-prompt}]
...
The function \texttt{query\_gemini\_cli()} builds a unified text prompt combining the system and user contexts, executes the \texttt{gemini} command via subprocess.
...
\paragraph{Local LLM Backends}

Local \glspl{llm}, particularly open-source models run through systems such as Ollama, do not provide native support for the \gls{mcp} protocol.
...
To ensure protocol-correct behaviour, a dedicated \emph{shimming layer} was introduced.
...
A detailed description of this mechanism is provided in Chapter~\ref{chp:mcp-shimming}.
...
Session management is handled by an helper module \texttt{jadx\_helper\_functions.py}, which contains utilities to start, monitor, and terminate the Jadx process.\\
...
For cloud-based \gls{llm} backends (e.g.\ GPT-5.1 or Gemini-3), integration with \gls{mcp} servers can rely directly on the Pydantic AI framework\footnote{\url{https://docs.pydantic.dev/latest/}}.  
Pydantic AI provides the \texttt{MCPServerStdio} utility, which allows the orchestration layer to launch and manage \gls{mcp} servers as subprocesses, including the startup command and initialisation timeout.

Listing~\ref{lst:jadxServer} shows the initialisation of the Jadx \gls{mcp} server, where an \texttt{MCPServerStdio} instance is created to launch \texttt{jadx\_mcp\_server.py} from the directory specified by the system variable \texttt{JADX\_MCP\_DIR}.
...
\paragraph{Shimming layer.} \label{chp:mcp-shimming}

The shimming agent wraps the open-source model in a supervised execution loop that enforces deterministic, protocol-correct behaviour.  
Its role is to ensure that models without native \gls{mcp} support can still interact reliably with external tools by constraining their outputs to a strict, verifiable format.
...
\begin{myenumerate}
    \item the model receives the available tools through the system prompt;
    \item then proposes a tool call in JSON form;
    \item the shimmer forwards the request to the appropriate \gls{mcp} server (Jadx or Ghidra);
    \item the server returns structured evidence;
    \item the shimmer returns this evidence to the model as the next input;
...
The purpose of the shimming layer is to mediate the interaction in a way that prevents protocol violations and ensures that local open-source models behave like fully \gls{mcp}-compatible agents.
...
\begin{lstlisting}[caption={Pseudo-prompt of the shimming layer, label={lst:pseudoPrompt}}]
...
If additional libraries become relevant during the analysis, the agent can still access them on demand through the \texttt{open\_external\_method} \gls{mcp} function.  
This mechanism allows the \gls{llm} to load previously excluded native binaries autonomously, ensuring that methods outside the initial import set can be inspected without manual intervention.
...
\begin{enumerate}
    ...
    \item \textbf{Construction of a filtered call graph.}  
    Starting from the complete FlowDroid output, only the call sequence that reaches the JNI bridge associated with the crash are kept.
    The result is a compact filtered \gls{jcg} containing the precise Java-level call chain leading to the native entry point.
    %To avoid loading the entire multi-gigabyte JSON structure in memory, the extraction is performed through targeted pattern searches (implemented using \texttt{rg}), which isolate only the call edges relevant to the crash path.  
