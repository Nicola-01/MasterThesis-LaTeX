...
Different backends, cloud-based services such as GPT or Gemini, the command-line \texttt{gemini-cli} interface, or local offline engines such as Ollama, require different integration strategies.  
...
However, the Ghidra MCP server only becomes available once the imported binary is opened inside the CodeBrowser (the main interface opened by the dragon icon), and this action cannot be performed via the command line.
...
\begin{itemize}
    ...
    This method allows the triage agent to discover which binaries can be analysed before issuing analysis-related tool calls.  
    ...
    This operation does not trigger analysis itself: it merely brings the selected binary into focus so that subsequent MCP calls operate on the intended file.
...
The Ghidra \gls{mcp} server exposes a variety of analysis and decompilation tools accessible to the triage agent through structured \gls{mcp} calls.  
...
\subsection{Extraction of Native Libraries}
...
\subsection{Java Call Graph}

A second input provided to the model through the \texttt{CrashSummary} object is the Java call graph.  
...
While the native stack trace already describes the execution path within the \texttt{.so} file, the Java call graph complements it with the higher-level control flow within the application.

The extraction is performed in two phases:

\begin{enumerate}
    \item \textbf{Generation of a global flow graph.}  
    A complete flow graph of the application is first generated using FlowDroid, which statically analyses the APK and produces a comprehensive call graph covering all possible execution paths.  
    The resulting structure is stored in a large JSON file.  
    \textcolor{red}{(Non so se devo essere più dettagliato o mettere il riferimento al codice, non so da dove provenga, quindi non ho un github, ho usato quello di Samuele)}
    ...
    The result is a compact, filtered call graph that contains only the calls involved in the crash path.
\end{enumerate}

This filtered Java call graph is then attached to the \texttt{CrashSummary}, enabling the \gls{llm} to reason about both native- and Java-level execution paths.  
