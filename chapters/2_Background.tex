...
\subsection{Ghidra}
Ghidra is an open-source software reverse-engineering framework developed by the NSA. It provides disassembly, decompilation, graphing, and scripting across multiple architectures, and is widely used in vulnerability research and malware analysis \cite{ghidra-github}. In the pipeline, Ghidra is used for Android native libraries packaged as ELF shared objects (\texttt{.so}) inside APKs (e.g., \texttt{lib/arm64-v8a/*.so}), extracting functions, cross-references, and decompiled snippets to triage \gls{jni}-mediated crashes.
