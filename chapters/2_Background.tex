...
\begin{lstlisting}[language=C+JNI, caption={Native implementation of \texttt{bar}\\The native function name \texttt{Java\_pkg\_foo\_bar} follows the JNI name-mangling convention: it is composed of the prefix \texttt{Java\_}, the fully qualified class name (\texttt{pkg.foo}, with dots replaced by underscores), and the Java method name \texttt{bar}, thus uniquely identifying the native implementation of \texttt{foo::bar(int,String)}.}, label={lst:JNI}]
...
%\subsection*{Uses}
Fuzzing is primarily employed to discover security-relevant defects in safety- or security-critical software. Its strength lies in \emph{demonstrating the presence} of bugs via concrete, reproducible inputs. Proving correctness for all inputs requires a formal specification and formal methods; fuzzing complements, rather than replaces, such approaches \cite{sutton2007fuzzing}.
...
An \gls{mcp} \emph{client} is a protocol-level component created by the host to communicate with exactly one \gls{mcp} \emph{server}. Each client maintains a transport (stdio or HTTP/Web Socket), negotiates capabilities, and exposes the server’s tools/resources/prompts to the host. This indirection lets the host manage several servers in parallel without coupling UI logic to any server implementation details \cite{mcp-client-concepts}. 
...
An \gls{mcp} \emph{server} wraps external systems (e.g. files, databases, reverse-engineering tools) and presents them as protocol objects. It exposes\textit{tools}, which expose invokable functions with JSON Schemas; \textit{resources}, which provide readable artefacts; and \textit{prompts}, which offer reusable task templates. Servers publish a capability manifest so that clients can discover what is available and how to access it. The \gls{mcp} \emph{server} provides tools that enable \glspl{llm} to perform actions, carry out deterministic computations and interact with external services. 
...
\begin{myenumerate}
  ...
  \item The server executes the tool (e.g., get app manifest with Jadx, decompile a function with Ghidra) and returns structured results to the client. % tool exec
...
In modern security analysis, vulnerabilities are not only discovered but must also be described, categorised, and prioritised in a consistent manner.  
To support this process, widely adopted standardised frameworks provide identifiers, taxonomies, and severity metrics that allow tools, researchers, and vendors to communicate about security issues in a uniform way.

\subsection{\glsxtrlong{cwe} (\glsxtrshort{cwe})}
...
Vulnerability triage is the process of assessing, prioritising, and deciding on the appropriate response for a set of identified software vulnerabilities~\cite{ThreatNG-vulnerability-triage}.  
In practice, this means evaluating each potential issue to determine if it represents a real security risk, estimating its impact and exploitability, and selecting which findings deserve immediate attention or remediation.

%In the context of this thesis, the goal is to automate the triage phase: given a crash in a native library (detected via fuzz testing or dynamic analysis), the system combines dynamic crash data and static-analysis context to output a structured verdict.  
%This verdict includes not only whether the crash likely corresponds to a vulnerability, but also severity, vulnerability classification (e.g.\ via CWE IDs), supporting evidence, and—when applicable—a plausible exploitation path or mitigation suggestions.  
...
\subsection{Ghidra}
Ghidra is an open-source software reverse-engineering framework developed by the National Security Agency (NSA). It offers disassembly, decompilation, graphing and scripting capabilities across multiple architectures, and is widely used in vulnerability research and malware analysis\cite{ghidra-github}.
In this thesis, it is used to allow the \gls{llm} to analyse and extract the information it needs from native binaries, thereby facilitating the crash triage of native libraries.

\subsection{Jadx}
Jadx is a decompiler that produces Java source code from Android DEX and APK files, providing both CLI and GUI front-ends. It is commonly used to inspect Android manifests, classes and resources\cite{jadx-github}.  
In the pipeline, Jadx is responsible for recovering APK metadata (e.g., SDK version) and analysing the Java code leading up to the JNI boundary.

\end{comment}

\subsection{Ghidra}
Ghidra is an open-source software reverse-engineering framework developed by the National Security Agency (NSA). It offers disassembly, decompilation, graphing and scripting capabilities across numerous architectures and binary formats, and is broadly adopted within vulnerability research and malware analysis communities \cite{ghidra-github}.  
In this thesis, it is used to enable the \gls{llm} to inspect native binaries: once an ELF library is imported, Ghidra can generate control-flow graphs, symbol tables, cross-references, and decompiled C code. These artefacts are then exposed via the \gls{mcp} server, allowing the pipeline to query functions, addresses, call‐graphs and other fine-grained details necessary for crash triage.

\subsection{Jadx}
Jadx is a decompiler that converts Android DEX and APK files into Java source code, offering both \gls{cli} and \gls{gui} front-ends \cite{jadx-github}.  
In the pipeline, Jadx fulfills two key roles: first, it recovers application metadata (such as the manifest, SDK version, permissions and package structure); second, it decompiles the Java portion of the application, enabling analysis of Java-to-JNI transitions.  