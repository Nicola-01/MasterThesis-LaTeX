...
Section~5.6 of the POIROT paper~\cite{poirot-usenix25} describes a reproducible use-after-free vulnerability in the \texttt{libTPMp4Encoder.so} library.  
The flaw is triggered when the application invokes the MP4 encoding pipeline with malformed JPEG or H.264 data:

\begin{enumerate}
    \item The JNI function \texttt{packVideo} receives attacker-controlled frame data and a size parameter.
    \item The native function \texttt{mp4\_write\_one\_h264} tears down the encoder context on malformed input: it closes the file, frees multiple internal buffers, and then frees the global structure \texttt{iniPacker\_global}.
...
The vulnerability has practical security impact: an attacker on the same network, or one who has compromised a TP-Link camera, can exploit the issue as soon as the user presses the “record” button in the tpCamera app. % and was validated both locally and through a MITM attack against the tpCamera application.

\subsection{\glsxtrshort{llm} Classification Summary}

The \gls{llm}-based triage correctly identifies the crash in \texttt{tpCamera} as a genuine vulnerability, assigning a \emph{high} severity level and a confidence score of 0.85.  
...
The full classification output for this application is provided in Appendix~\ref{chp:appendixB}. The analysis points to three main contributing factors:
...
Both sources identify the vulnerability as a use-after-free condition arising from repeated invocations of \texttt{packVideo} on a freed encoder context, although the \gls{llm} expresses the use-after-free through its \gls{cwe} assignment rather than presenting it as the primary causal mechanism.
...
This comparison shows that the \gls{llm}-based triage is capable of reconstructing the essential characteristics of a complex native vulnerability using stack-trace evidence, decompiled code, and contextual information accessed through \gls{mcp} tools.  
...
The analysis produced by the \gls{llm} is not only consistent with the POIROT ground truth, but also captures several realistic exploitation vectors. Demonstrates that the proposed pipeline can approximate expert-level reasoning and align closely with real-world vulnerability reports, validating its effectiveness as an initial triage mechanism.
