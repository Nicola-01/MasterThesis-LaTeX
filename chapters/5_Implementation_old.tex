...
\textcolor{red}{Da rivedere quando il capitolo è finito, non so se va in Design}
...
\texttt{CrashSummary} represents a single crash entry produced by POIROT.  
It contains the parsed information from the crash report, including the process termination cause, the native stack trace, the \gls{jni} bridge method, the fuzzing harness entry point, and the reconstructed Java call graph.  

\texttt{AppMetadata} stores the metadata extracted from the APK manifest, such as the application name, package identifier, SDK versions, and the APK version .

\texttt{VulnResult} models the outcome of the vulnerability assessment performed on a single crash.  
It encodes whether the crash is likely to represent a real vulnerability, the associated confidence score, the set of supporting reasons, any relevant \gls{cwe} identifiers, and an estimated severity level.  
It also records the affected native libraries, a list of concrete evidence items (e.g.\ function names, addresses, code snippets), recommended remediation steps, and the assumptions or limitations considered during the analysis.

\paragraph{Prompt specialisation.}
Under \texttt{MCPs/prompts/}, each prompt file encodes a distinct component of the LLM orchestration logic and specialises the model for a specific task within the triage pipeline.  

\texttt{AppMetadata\_prompt.py} provides the rules for extracting manifest and resource-level metadata from Jadx, including package name, SDK levels, and application labels.
  
\texttt{Shimming\_prompts.py} defines the system prompts that enforce MCP protocol correctness (single JSON per turn, tool-call constraints, and multi-step decompilation workflows). These templates merge the Jadx and Ghidra toolsets and dynamically inject the correct output schema depending on whether the target task is metadata extraction or vulnerability assessment.

\texttt{VulnDetection\_prompt.py} encodes the full triage specification: the definition of what qualifies as a vulnerability, the backward data-flow reasoning procedure, the exploitation feasibility checks, and the structured JSON schema used.
...
This configuration was introduced to bypass \gls{api} usage quotas and benefit from the higher request limits available with \texttt{gemini-2.5-pro}. \textcolor{red}{Da lasciare la specificazione di perchè è stato fatto?}
...
Open-source \glspl{llm} typically understand the \emph{concept} of tool invocation (i.e.\ they can follow JSON-based calling rules) but they do not natively implement the \gls{mcp} nor guarantee strict compliance with its turn-based constraints. The shimming layer acts as an adapter between the model and the \gls{mcp} toolsets (Jadx MCP and Ghidra MCP).

The shim enforces:
\begin{itemize}
    \item strict ``one JSON object per turn'' interaction;
    \item automatic parsing and validation of the model’s JSON output;
    \item redirection of each tool response back to the model as the next prompt.
\end{itemize}

Operationally, the model never interacts with \gls{mcp} endpoints directly.  
Instead, the \texttt{oss\_model} function receives the system prompt generated from the shimming templates, asks the \gls{llm} to output a JSON tool call, executes that call on behalf of the model, and feeds the result back as a ``Tool~Response''.  
This loop continues until the model emits a valid \texttt{"action": "final"} JSON object matching the expected schema.

This mechanism provides robust tool-calling support even for models that lack native \gls{mcp} integration, while guaranteeing protocol correctness and shielding the model from the complexity of asynchronous tool execution.
...
\begin{figure}
    ...
    \caption{Directory structure of the target APKs as produced by POIROT, including extracted libraries and reproduced crash outputs.}
...
The triage tool uses a preference list of \glspl{abi} to select the variant to analyse first; for example, it prioritises \texttt{arm64-v8a}.
...
\textcolor{red}{added call\_sequence and Exploit}
...
\begin{itemize}
  ...
  \item \textbf{\texttt{analysis.libs}}  
  A list of native libraries that were considered relevant for the apk run.  
...
%Security analysts can directly inspect the rationales and evidence for each crash, while downstream tools can parse the JSON to aggregate statistics, feed dashboards, or gate CI pipelines on the presence of high-severity vulnerabilities.
