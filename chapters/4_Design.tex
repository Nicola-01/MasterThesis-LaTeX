...
In addition to the native stack trace, the \texttt{CrashSummary} includes the filtered \gls{jcg} leading to the JNI entry point, as well as the mapping of libraries and methods involved in the fault.  
...
\begin{myitemize}
    ...
    \item \textbf{confidence}: a calibrated score in the range $[0,1]$ reflecting how strongly the evidence supports the classification.
...
Two additional components—\textbf{Chain-of-Thought (CoT)} and the \textbf{Exploit / Chain-of-Process (CoP)} block—are included to guide the \gls{llm} toward grounded, non-speculative reasoning.
...
By prompting the model to reason explicitly, the system reduces the likelihood of superficial or hallucinated assessments and encourages the analysis of concrete evidence retrieved via \gls{mcp} tools.  
