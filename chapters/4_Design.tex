...
\textcolor{red}{Gestione del GhidraMCP e modifiche a Jadx MCP}
...
\end{itemize}
These tool calls allow the \gls{llm} to anchor its analysis in concrete information extracted directly from the application, thereby reducing hallucinations and enabling a more reliable, evidence-driven vulnerability assessment. \textcolor{red}{Qui mi baso sul fatto che alcuni paper parlavano che agli llm manca il contesto}

\subsection{Evidence Integration and Final Report Generation}

In the final stage, the \gls{llm} aggregates all retrieved evidence and synthesises a structured vulnerability report.  
The model combines information extracted from the crash context, the Java call graph, and the native and Java artefacts accessed via MCP tools.  
%This step is critical because it consolidates heterogeneous data sources into a unified representation that reflects both the dynamic behaviour of the crash and the static structure of the application.

The output follows a constrained JSON schema and includes fields such as the vulnerability verdict, confidence score, severity, CWE identifiers, affected libraries, reasoning steps, code evidence, assumptions, and limitations.  
Each field is populated according to explicit rules defined in the system-level meta-prompt: for example, The confidence score should reflect the degree to which the classification is considered accurate; evidence items must reference concrete function names, addresses, or decompiled snippets; and assumptions or limitations must be stated whenever the available artefacts are incomplete or ambiguous.

The final report is entirely machine-readable and encapsulates both the analytical process and the supporting evidence.  
This makes it suitable not only for manual inspection by security analysts but also for automated downstream processing, such as filtering high-severity findings, or generating aggregated statistics across multiple applications.  
The JSON structure therefore serves as the primary output of the triage pipeline, providing a consistent interface for downstream analysis, auditing, and reproducibility.
